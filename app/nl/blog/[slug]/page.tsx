...
import {
  BLOG_ARTICLES_NL,
  BLOG_ARTICLE_SLUGS_NL,
  type BlogArticleSlugNl,
...
function resolveArticlePageId(slug: string): PageId | null {
  if (slug in BLOG_ARTICLES_NL) {
    return BLOG_ARTICLES_NL[slug as BlogArticleSlugNl];
  }
  return null;
}

function getLocalizedContent(pageId: PageId) {
  return localizeBlogArticleContent(getPageContent(pageId), "nl", pageId);
}

export function generateStaticParams() {
  return BLOG_ARTICLE_SLUGS_NL.map((slug) => ({ slug }));
}
...
export default async function NlBlogArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const pageId = resolveArticlePageId(slug);
  if (!pageId) notFound();
