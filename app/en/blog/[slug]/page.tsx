...
import {
  BLOG_ARTICLES_EN,
  BLOG_ARTICLE_SLUGS_EN,
  type BlogArticleSlugEn,
...
function resolveArticlePageId(slug: string): PageId | null {
  if (slug in BLOG_ARTICLES_EN) {
    return BLOG_ARTICLES_EN[slug as BlogArticleSlugEn];
  }
  return null;
}

function getLocalizedContent(pageId: PageId) {
  return localizeBlogArticleContent(getPageContent(pageId), "en", pageId);
}

export function generateStaticParams() {
  return BLOG_ARTICLE_SLUGS_EN.map((slug) => ({ slug }));
}
...
export default async function EnBlogArticlePage({ params }: PageProps) {
  const { slug } = await params;
  const pageId = resolveArticlePageId(slug);
  if (!pageId) notFound();
